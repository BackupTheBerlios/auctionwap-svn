...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim titl As String
        If Request.QueryString.Get("titl") <> "" Then
            titl = Request.Form.Get("titl")
        End If
        If Request.QueryString.Get("TextBox1") <> "" Then
            titl = Request.QueryString.Get("TextBox1")
        End If
        Label1.Text = titl
        ...
        End If
        If titl = "" Then titl = " "
        Sql = "select Itemid,title, cbid,  DateDiff('h',Date(),DateAdd('d',items.duration,items.SDt)), NoofBids from items where DateDiff('h',Date(),DateAdd('d',items.duration,items.SDt)) > 0 and instr(1,ucase(items.title),'" & UCase(titl) & "') "
        ...
        If Dr.HasRows Then
            If Dr.Read() Then
                item1.Visible = True
                item1.Text = "1." & Dr.GetValue(1)
                item1.NavigateUrl = "WapBid.aspx?itemid=" & Dr.GetValue(0)
                rCount = rCount + 1
            Else
                item1.Visible = False
            End If
            If Dr.Read() Then
                item2.Visible = True
                item2.Text = "2." & Dr.GetValue(1)
                item3.NavigateUrl = "WapBid.aspx?itemid=" & Dr.GetValue(0)
                rCount = rCount + 1
            Else
                item2.Visible = False
            End If
            ...
            If Dr.Read() Then
                item4.Visible = True
                item4.Text = "4." & Dr.GetValue(1)
                item4.NavigateUrl = "WapBid.aspx?itemid=" & Dr.GetValue(0)
                rCount = rCount + 1
            Else
                item4.Visible = False
            End If
            If Dr.Read() Then
                item5.Visible = True
                item5.Text = "5." & Dr.GetValue(1)
                item5.NavigateUrl = "WapBid.aspx?itemid=" & Dr.GetValue(0)
                rCount = rCount + 1
            Else
                item5.Visible = False
            End If
        ...
        If CurrentPage < PageCount Then
            ...
            lnkNext.NavigateUrl = "finder.aspx?cp=" & CurrentPage & "&titl=" & titl
        Else
            lnkNext.Visible = False
        End If
    End Sub

    Private Sub ObjectList1_ItemCommand(ByVal sender As System.Object, ByVal e As System.Web.UI.MobileControls.ObjectListCommandEventArgs)
